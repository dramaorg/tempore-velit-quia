## @dramaorg/tempore-velit-quia &mdash; object viewer &nbsp; &nbsp; [![npm](https://img.shields.io/npm/v/@dramaorg/tempore-velit-quia.svg)](https://www.npmjs.com/package/@dramaorg/tempore-velit-quia) [![build status](https://github.com/dramaorg/tempore-velit-quia/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/dramaorg/tempore-velit-quia/actions/workflows/ci.yml)

**@dramaorg/tempore-velit-quia** &mdash; pronounced *vúed* or *vue dd* &mdash;  is a Vue 3 object viewer for displaying complex JavaScript objects, Vue Reactive and Ref objects, Functions, Maps, Sets, Symbols and all JavaScript Primitives in order to monitor their changes **live** with full reactivity. Inspired by the Laravels **dd()** debugging function.
...
```js
import { VueDd } from '@dramaorg/tempore-velit-quia'
...
```bash
# npm
npm install @dramaorg/tempore-velit-quia
# yarn
yarn add @dramaorg/tempore-velit-quia
```
...
```html
<@dramaorg/tempore-velit-quia v-model="object" />
```
...
- performance optimized, even large objects like `window` can be @dramaorg/tempore-velit-quia
...
| open   | `(setup) => {...}` | `{ open, pointer, level } = setup`                                             | On open by @dramaorg/tempore-velit-quia itself or by a user, this emit will be called |
...
- There are many things that can be done with this package, for ex. here is what I am doing with it right now:
    ...
    - All of these features area currently in development in **XRay** component, that will soon be part of **@dramaorg/tempore-velit-quia**, but feel free to develop your own versions with this package
...
Versions up to **@dramaorg/tempore-velit-quia@1.1.6** supported **Vue 2.7.5** and above, but import of the package is different from Vue 3. In Vue 2 @dramaorg/tempore-velit-quia is imported as default and not as a destructured `VueDd`
```js
// to install @dramaorg/tempore-velit-quia with Vue 2 support
// yarn add @dramaorg/tempore-velit-quia@1.1.6
// npm install @dramaorg/tempore-velit-quia@1.1.6
// note: 'import VueDd...', and not 'import { VueDd }...'
import VueDd from '@dramaorg/tempore-velit-quia'
```
Also note in Vue 2, you have to use `:model-value` instead of `v-model`
```html
<@dramaorg/tempore-velit-quia :model-value="obj" />
```
> Note that Vue 2 version cannot support large objects like `window`. For some reason, it reaches a stack size limit and breaks. It is a known issue in Vue 2, that's why migrate to Vue 3 ASAP in order to use the full power of **@dramaorg/tempore-velit-quia**
## Credits & Thanks
...
> **Highlight.js** is built into @dramaorg/tempore-velit-quia to provide JavaScript function highlighting.
